...
class _BrainAnalyticsScreenState extends State<BrainAnalyticsScreen> {
  ...
  void initState() {
    super.initState();
    BlocProvider.of<EvaluationMindQuestionResultBloc>(context).add(FetchEvaluationMindQuestionResultEvent());
    BlocProvider.of<EvaluationMindResultBloc>(context).add(FetchEvaluationMindResultEvent());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const AdaptiveAppBar(null, 'Evaluation Mind'),
      body: BlocBuilder<EvaluationMindResultBloc, EvaluationMindResultState>(
        builder: (context, state) {
          ...
          if (state is EvaluationMindResultLoaded) {
            List<EvaluationMindResult> dataList = state.evaluationMindResultList;
            if (dataList.isEmpty) {
              return const Center(
            ...
            print(leftValue);
            print(rightValue);
            return BlocBuilder<EvaluationMindQuestionResultBloc, EvaluationMindQuestionResultState>(
              builder: (context, state) {
                if (state is EvaluationMindQuestionResultLoaded) {
                  List<EvaluationMindQuestionResult> list = state.evaluationMindQuestionResultList;
                  if (list.isEmpty) {
                    return const Center(
                      child: Text('No result found'),
                    );
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(
                              width: width * 0.8,
                              height: height * 0.3,
                              child: leftValue > rightValue ? Image.asset(leftBrainIcon) : Image.asset(rightBrainIcon),
                            ),
                            const SizedBox(
                              height: 20,
                            ),
                            getBrainCardResult(
                              width,
                              height,
                              leftValue > rightValue ? Style.pColor : Colors.orange[800],
                              resultList,
                            ),
                            // const SizedBox(
                            //   height: 10,
  ...
  Widget getBrainCardResult(double width, double height, Color? color, List<EvaluationMindQuestionResult> list) {
    return Container(
      width: width * 0.7,
      height: height * 0.18,
      decoration: BoxDecoration(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    children: List.generate(
                      list.length,
                      (index) => getStatusRow("${list[index].questionStatement} :", list[index].mindintensity, constraints),
                    ),
                    // children: [
